...
void SR300ParamManager::callback(RealSenseNode* node_ptr, sr300_paramsConfig &config, uint32_t level)
{
    ...
    if (node_ptr->set_default_dynamic_reconfig_values == level)
    {
        for (int i = 1 ; i < sr300_param_count ; ++i)
        {
            ...
            setParam(node_ptr, config ,(sr300_param)i);
        }
    }
    else
    {
        setParam(node_ptr, config, (sr300_param)level);
    }
}
